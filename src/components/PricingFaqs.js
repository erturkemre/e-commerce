import { ChevronRight } from "lucide-react";
import React from "react";

const PricingFaqs = () => {
  return (
    <div>
      <div className="flex flex-col gap-[50px] py-20 items-center">
        <div className="py-[45px] flex flex-col gap-[10px]  text-center">
          <h2 className="font-bold text-[40px] text-[#252B42]">Pricing FAQs</h2>
          <p className="font-normal text-xl text-[#737373] px-0 sm:px-96 ">
            Problems trying to resolve the conflict between the two major realms
            of Classical physics
          </p>
        </div>
        <div className="flex flex-col items-center w-[80%] gap-10 sm:gap-[30px] container m-auto justify-center sm:px-24 px-0">
          <div className="flex flex-col sm:flex-row items-center gap-10 sm:gap-[30px] ">
            <div className="flex gap-5 p-6 ">
              <ChevronRight className="w-20" />
              <div className="flex flex-col">
                <h5 className="text-[#252B42] text-base font-bold">
                  the quick fox jumps over the lazy dog
                </h5>
                <p className="font-normal text-sm text-[#737373]">
                  Met minim Mollie non desert Alamo est sit cliquey dolor do met
                  sent. RELIT official consequent door ENIM RELIT Mollie.
                  Excitation venial consequent sent nostrum met.
                </p>
              </div>
            </div>
            <div className="flex gap-5 p-6 ">
              <ChevronRight className="w-20" />
              <div className="">
                <h5 className="text-[#252B42] text-base font-bold">
                  the quick fox jumps over the lazy dog
                </h5>
                <p className="font-normal text-sm text-[#737373]">
                  Met minim Mollie non desert Alamo est sit cliquey dolor do met
                  sent. RELIT official consequent door ENIM RELIT Mollie.
                  Excitation venial consequent sent nostrum met.
                </p>
              </div>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-center gap-10 sm:gap-[30px] ">
            <div className="flex gap-5 p-6">
              <ChevronRight className="w-20" />
              <div className="">
                <h5 className="text-[#252B42] text-base font-bold">
                  the quick fox jumps over the lazy dog
                </h5>
                <p className="font-normal text-sm text-[#737373]">
                  Met minim Mollie non desert Alamo est sit cliquey dolor do met
                  sent. RELIT official consequent door ENIM RELIT Mollie.
                  Excitation venial consequent sent nostrum met.
                </p>
              </div>
            </div>
            <div className="flex gap-5 p-6">
              <ChevronRight className="w-20" />
              <div className="">
                <h5 className="text-[#252B42] text-base font-bold">
                  the quick fox jumps over the lazy dog
                </h5>
                <p className="font-normal text-sm text-[#737373]">
                  Met minim Mollie non desert Alamo est sit cliquey dolor do met
                  sent. RELIT official consequent door ENIM RELIT Mollie.
                  Excitation venial consequent sent nostrum met.
                </p>
              </div>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-center gap-10 sm:gap-[30px] ">
            <div className="flex gap-5 p-6">
              <ChevronRight className="w-20" />
              <div className="">
                <h5 className="text-[#252B42] text-base font-bold">
                  the quick fox jumps over the lazy dog
                </h5>
                <p className="font-normal text-sm text-[#737373]">
                  Met minim Mollie non desert Alamo est sit cliquey dolor do met
                  sent. RELIT official consequent door ENIM RELIT Mollie.
                  Excitation venial consequent sent nostrum met.
                </p>
              </div>
            </div>
            <div className="flex gap-5 p-6">
              <ChevronRight className="w-20" />
              <div className="">
                <h5 className="text-[#252B42] text-base font-bold">
                  the quick fox jumps over the lazy dog
                </h5>
                <p className="font-normal text-sm text-[#737373]">
                  Met minim Mollie non desert Alamo est sit cliquey dolor do met
                  sent. RELIT official consequent door ENIM RELIT Mollie.
                  Excitation venial consequent sent nostrum met.
                </p>
              </div>
            </div>
          </div>
        </div>
        <p className="text-center px-12 sm:px-0 text-xl font-normal text-[#737373]">
          Haven’t got your answer? Contact our support
        </p>
      </div>
    </div>
  );
};
export default PricingFaqs;
